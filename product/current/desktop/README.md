...
The desktop shell imports the official profile boot module from its embedded runtime closure and mounts the unmodified Web profile in the Electron main process. The profile's default-browser handoff is disabled through its official command-line service, and the shell loads the resulting internal loopback URL in the renderer. Port `3080` is reserved for this integrated application; the shell never attaches to an unrelated process already using it. `DSH_DESKTOP_HOST_URL` is available only for development diagnostics. Child tools launched by the official runtime inherit Electron's Node-compatible mode through `ELECTRON_RUN_AS_NODE=1`.
