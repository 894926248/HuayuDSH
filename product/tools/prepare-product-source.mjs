...
const outputRoot = resolve(process.argv[2] ?? resolve(root, '.workspace/artifacts/staging/source'))
const overlayRoot = resolve(root, 'product/app')
...
async function removeWorktree() {
  ...
  try { git(upstreamRoot, ['worktree', 'prune'], { stdio: 'ignore' }) } catch {}
}
...
async function applyProductOverlay(outputRoot, manifest) {
  for (const row of manifest.files) {
    ...
    if (!row.path.startsWith('apps/web/') && !row.path.startsWith('packages/client/')) {
      throw new Error(`product overlay target is outside the WebUI plane: ${row.path}`)
    }
...
async function main() {
  const expected = readUpstreamRelease(root)
  ...
  if (expected.commit !== actual) throw new Error(`upstream source changed during preparation: expected ${expected.commit}, found ${actual}`)
