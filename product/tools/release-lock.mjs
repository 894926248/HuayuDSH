...
const versionPath = resolve(root, 'product/config/product-version.json')
const artifactsRoot = resolve(root, '.workspace/artifacts')
const defaultArtifactRoot = resolve(artifactsRoot, 'staging/current')
...
function requireCleanReleaseSources() {
  ...
  const upstreamRelease = readUpstreamRelease(root)
  if (upstreamRelease.commit !== upstream.commit) throw new Error(`upstream source changed during release: ${upstreamRelease.commit} != ${upstream.commit}`)
  return { product, upstream, upstreamRelease }
}
...
async function writeLock(values) {
  ...
  assertInside(artifactsRoot, artifactRoot, 'artifact root')
  assertInside(artifactsRoot, output, 'manifest')
  ...
  const manifest = {
    ...
    upstream: { version: sources.upstreamRelease.version, tag: sources.upstreamRelease.tag, commit: sources.upstream.commit, tree: sources.upstream.tree },
    artifacts,
  }
...
async function verifyLock(values) {
  const manifestPath = resolve(root, option(values, '--manifest', relative(root, resolve(artifactsRoot, 'active/release-lock.json'))))
  assertInside(artifactsRoot, manifestPath, 'manifest')
  ...
  for (const [label, expected, actual] of [
    ['upstream version', manifest.upstream.version, sources.upstreamRelease.version],
    ['upstream tag', manifest.upstream.tag, sources.upstreamRelease.tag],
