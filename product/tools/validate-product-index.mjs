...
const indexPath = resolve(root, 'product/config/changes.json')
...
for (const change of registry.changes ?? []) {
  ...
  const productOwner = typeof change.owner === 'string'
    && change.owner.startsWith('product/')
    && !change.owner.startsWith('product/config/')
  const pluginOwner = typeof change.owner === 'string' && change.owner.startsWith('plugins/')
  if (change.owner !== 'upstream' && !productOwner && !pluginOwner) {
    errors.push(`${change.id}: owner must be upstream, a product application/tool directory, or a plugins directory`)
  } else if (!existsSync(resolve(root, change.owner))) {
    errors.push(`${change.id}: owner path is missing: ${change.owner}`)
  }
  if (!Array.isArray(change.sourcePaths) || change.sourcePaths.some(path => typeof path !== 'string' || path.startsWith('/') || path.includes('..') || path.startsWith('.workspace/'))) {
    errors.push(`${change.id}: sourcePaths must contain repository-relative paths`)
  } else {
    for (const path of change.sourcePaths) {
      if (!existsSync(resolve(root, path))) errors.push(`${change.id}: source path is missing: ${path}`)
    }
  }
