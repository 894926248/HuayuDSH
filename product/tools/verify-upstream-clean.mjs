...
const upstreamTag = git(upstream, ['describe', '--tags', '--exact-match', 'HEAD'])
const upstreamStatus = git(upstream, ['status', '--porcelain', '--untracked-files=all'])
const upstreamRelease = readUpstreamRelease(root)
if (upstreamRelease.commit !== upstreamCommit || upstreamRelease.tag !== upstreamTag) {
  console.error(`upstream version mismatch: package=${upstreamRelease.version}, tag=${upstreamTag}, commit=${upstreamCommit}`)
  process.exitCode = 1
}
if (upstreamStatus !== '') {
...
if (productOnlyRepository) {
  console.log(`product-only repository: upstream ${upstreamRelease.version} (${upstreamTag}, ${upstreamCommit}) is clean`)
