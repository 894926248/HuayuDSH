...
任务栏和窗口使用同一套 DSH 原始图标 `apps/web/public/favicon.svg` 的浅色／深色栅格资源，并按渲染器解析出的主题切换。便携版可执行文件和快捷方式继续使用由同一 DSH 资源生成的稳定 `build/icon.ico` 身份，因此切换主题不会改变 exe 或快捷方式目标。便携包会把完整官方运行时闭包放到 `resources/runtime`，`host-root.json` 只保存指向包内运行时的相对路径；发布后不依赖源码检出目录或 `.workspace`。Electron 壳会从包内运行时启动原始的 `apps/cli/lib/bin.js` 子进程，保留官方 Profile 的模块解析和生命周期边界；它请求操作系统分配 loopback 端口，加载 CLI 宣布的地址，并在壳退出时终止自己拥有的子进程。打包会逐字节比较指针，并启动包内运行时验证临时 `DSH_HOME` 及 Web HTML。
...
桌面壳会从源码检出目录或包内运行时启动原始 Web CLI 入口，以独立子进程运行 Web Profile，关闭 Profile 默认打开系统浏览器的行为，请操作系统分配一个可用的 loopback 端口，再由渲染器加载 CLI 宣布的地址。它不会自动附着到其他进程占用的端口。`DSH_DESKTOP_HOST_URL` 可以显式指定已经运行的 loopback Web 宿主；`DSH_DESKTOP_SOURCE_ROOT` 和 `DSH_DESKTOP_HOST_PATH` 仍可覆盖开发环境的宿主根目录及入口。Host 启动前由 Electron 解析系统代理，并把 `HTTP_PROXY`、`HTTPS_PROXY`、`ALL_PROXY`、`NO_PROXY` 和 `NODE_USE_ENV_PROXY=1` 传给官方 Node 子进程，因此 Host 与外壳使用同一套系统代理。Profile 的用户插件由官方 CLI 在 `DSH_HOME` 下按原有回退闭包解析，Electron 只负责拥有并回收该子进程。
