...
`desktop:package` creates an unpacked application directory under `.workspace/artifacts/staging/desktop`. `desktop:dist` creates the fixed-name Windows portable executable in the same staging directory. A release build must use `node product/tools/build-desktop-release.mjs`, which records the result under `.workspace/artifacts/releases/<version>/` and copies the active executable to `.workspace/artifacts/active/`.
...
The fixed release file is `DeepSeek Harness.exe`. The portable launcher may run its extracted copy from `%TEMP%`; that path is an implementation detail and must not be used by a shortcut. Use `pnpm --filter @deepseek-ai/dsh-desktop run repair-shortcut` after a successful product release to target `.workspace/artifacts/active/DeepSeek Harness.exe`; pass additional existing `.lnk` paths when repairing taskbar pins. The script writes the stable target, icon, and `System.AppUserModel.ID` (`ai.deepseek.harness`) so the pinned shortcut and the running window stay in one taskbar group.

The native taskbar and window icon use paired light/dark rasterizations of the canonical Web `apps/web/public/favicon.svg`, selected from the renderer's resolved theme. The portable executable and shortcut keep the stable `build/icon.ico` identity derived from the same DSH asset, so theme changes do not alter the executable or shortcut target. The package carries the complete official runtime closure under `resources/runtime` and stores a relative `host-root.json` pointer to that package-local runtime. It does not depend on the source checkout or `.workspace` after release. The Electron shell launches the original `apps/cli/lib/bin.js` as a separate CLI process, preserving the official profile's module-resolution and lifecycle boundaries. It requests `--port 0`, loads the announced loopback URL, and terminates that owned child when the shell exits. Packaging compares the generated and unpacked pointer byte for byte, then starts the unpacked runtime with a temporary `DSH_HOME` and verifies its served HTML before publishing any artifact.
...
The desktop shell launches the original Web CLI entry as a separate Node child, disables the profile's default-browser handoff, asks the OS for an available loopback port, and loads that URL in the renderer. It never automatically attaches to a different process already using that port. The ordinary browser profile continues to default to `http://127.0.0.1:3080`; `DSH_DESKTOP_HOST_URL` can explicitly select an already running loopback Web host. `DSH_DESKTOP_SOURCE_ROOT` and `DSH_DESKTOP_HOST_PATH` override the source root and host entry for packaged deployments. Before the child starts, Electron resolves the system proxy for the Host and passes `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY`, `NO_PROXY`, and `NODE_USE_ENV_PROXY=1` to the official Node process. Backend child tools inherit Electron's Node-compatible executable mode, so existing `process.execPath` subprocess contracts continue to run as Node tools.
