...
let modelsDevBrowserCache: { expires: number; data: unknown } | undefined
...
function modelsDevRows(data: unknown, provider: string): DiscoveredModelView[] {
  ...
  for (const [id, value] of Object.entries(raw['models'])) {
    ...
    const limit = recordOf(value['limit']) ? value['limit'] : undefined
    const contextWindow = recordOf(limit) && typeof limit['context'] === 'number'
      ? limit['context'] : undefined
    const maxTokens = recordOf(limit) && typeof limit['output'] === 'number'
...
async function browserModelsDev(provider: string): Promise<readonly DiscoveredModelView[]> {
  const now = Date.now()
  if (modelsDevBrowserCache !== undefined && modelsDevBrowserCache.expires > now) {
    return modelsDevRows(modelsDevBrowserCache.data, provider)
  }
  ...
  modelsDevBrowserCache = { expires: now + 5 * 60 * 1000, data }
  return modelsDevRows(data, provider)
}

function supplementRows(
  rows: readonly DiscoveredModelView[],
  supplement: readonly DiscoveredModelView[],
): DiscoveredModelView[] {
  ...
  return rows.map(row => {
    ...
    return {
      ...row,
      ...row.name === undefined && extra.name !== undefined ? { name: extra.name } : {},
      ...row.contextWindow === undefined && extra.contextWindow !== undefined
        ? { contextWindow: extra.contextWindow } : {},
      ...row.maxTokens === undefined && extra.maxTokens !== undefined
...
export function ModelListEditor(props: ModelListEditorProps): ReactNode {
  ...
  const completeDirectory = async (
    ...
    if (probe.provider === undefined || rows.every(row => (
      row.name !== undefined && row.contextWindow !== undefined && row.maxTokens !== undefined
    ))) return rows
    try {
  ...
  // Existing settings may contain only a model id. Query the same shared
  // directory when the card opens so aliases and capacities appear without
  // requiring a separate button click. This never adds a new model and only
  // fills fields that the saved row does not already own.
  useEffect(() => {
    ...
    const hydrate = async (): Promise<void> => {
      try {
        ...
        if (stale) return
        const found = response.result.ok
          ? await completeDirectory(response.result.value.models)
          : probe.provider === undefined ? [] : await browserModelsDev(probe.provider)
        ...
        const hydrated = models.map(model => {
          ...
          return {
            ...model,
            ...model.name === undefined && candidate.name !== undefined ? { name: candidate.name } : {},
            ...model.contextWindow === undefined && candidate.contextWindow !== undefined
              ? { contextWindow: candidate.contextWindow }
              : {},
            ...model.maxTokens === undefined && candidate.maxTokens !== undefined
              ? { maxTokens: candidate.maxTokens }
              : {},
          }
        })
        if (hydrated.some((model, index) => JSON.stringify(model) !== JSON.stringify(models[index]))) {
          onChange(hydrated)
        }
      } catch {
        // Automatic enrichment is best effort; the explicit fetch action still
        // reports a useful failure when the user asks to inspect the directory.
      }
    }
    void hydrate()
    return () => { stale = true }
  // The initial rows are intentional: rerunning after onChange would create a
  // settings-write loop. Probe changes remount the provider editor in practice.
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [api.llm, probe.settingsNs, probe.provider, probe.baseURL, probe.api, probe.apiKey])
  ...
  const fetchModels = async (): Promise<void> => {
    ...
    try {
      ...
      // The endpoint catalog is complete, but only the models already saved
      // in this provider profile start checked. New models require an explicit
      // user selection before they enter settings.yaml.
      const known = new Set(models.map(model => textOf(model, 'id')))
      // Existing rows are not new selections. Fill only metadata they do not
      // already own so a refresh makes their alias and capacities visible in
      // the editor without overwriting user-tuned values.
      ...
      if (enrichedExisting.some((model, index) => JSON.stringify(model) !== JSON.stringify(models[index]))) {
  ...
  const adoptPicked = (): void => {
    ...
    for (const candidate of candidates) {
      ...
      if (existing === undefined) {
        ...
        // A saved row remains authoritative for edits, but a row that only
        // contains an id should still receive the directory's alias and
        // capacities. This is what makes an existing model show its context
        // and output limits after the catalog is refreshed.
        byId.set(candidate.id, {
          ...existing,
          ...existing.name === undefined && candidate.name !== undefined ? { name: candidate.name } : {},
          ...existing.contextWindow === undefined && candidate.contextWindow !== undefined
            ? { contextWindow: candidate.contextWindow }
            : {},
          ...existing.maxTokens === undefined && candidate.maxTokens !== undefined
            ? { maxTokens: candidate.maxTokens }
            : {},
        })
      }
