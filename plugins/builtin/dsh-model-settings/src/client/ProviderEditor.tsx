...
type EditorLayout = 'deepseek' | 'pi-ai' | 'generic' | 'unknown'
...
function layoutOf(
  ...
  if (Array.isArray(models) || schema.nodeAtPath(schema.rehydrate(namespace.schema), [...settingsPath, 'models']) !== undefined) {
    return 'generic'
  }
  return 'unknown'
}
...
export function ProviderEditor(props: ProviderEditorProps): ReactNode {
  ...
  const setBaseURL = (next: string): void => {
    const value = next.trim().length === 0 ? undefined : next
    setDraft(current => {
      const withoutLegacy = props.provider === 'commandcode'
        ? schema.deletePath(current, ['apiBase'])
        : current
      return value === undefined
        ? schema.deletePath(withoutLegacy, ['baseURL'])
        : schema.setPath(withoutLegacy, ['baseURL'], value)
    })
  }
  ...
  const probeBaseURL = stringAt(draft, 'baseURL')
    ?? (props.provider === 'commandcode' ? stringAt(draft, 'apiBase') : undefined)
    ?? stringAt(fallback, 'baseURL')
    ?? (props.provider === 'commandcode' ? stringAt(fallback, 'apiBase') : undefined)
  const probe = {
   ...
   */
  const curatedFields = (family: 'deepseek' | 'pi-ai' | 'generic'): ReactNode => {
    ...
    const keyPlaceholder = keyLocked
      ...
      : keyState?.configured === true && props.credentialRequired !== true
        ? t('keyStored')
        : family === 'pi-ai' || family === 'generic' ? t('keyPlaceholderNative') : t('keyPlaceholder')
