// ocgo-tab-ring — Client half (rich floating popup card, dsh-ocgo-lite style)
// 在“轨迹”标签右侧显示 ContextMeter 风格进度圈；鼠标悬停弹出 OpenCodeGo 用量卡片。
// 卡片通过 Portal 挂到 body，使用 position:fixed 定位，不会把标签栏撑出多行。
...
`
    ...
    function Card({ data, style, onRefresh, onClose }) {
      if (!data || !data.ok) {
        return React.createElement('div', { className: 'ocgo-tab-ring-card', style },
      ...
      return React.createElement('div', { className: 'ocgo-tab-ring-card', style }, children)
    }
    ...
    function TabRing() {
      ...
      const openCard = () => {
        ...
        const cardWidth = 300
        const left = Math.max(8, Math.min(rect.left + rect.width / 2 - cardWidth / 2, window.innerWidth - cardWidth - 8))
      ...
      const style = pos ? { left: pos.left, top: pos.top } : undefined
      ...
      return React.createElement('span', {
        ...
        open && pos ? ReactDOM.createPortal(
          React.createElement(Card, { data, style }),
          document.body,
        ) : null,
      )
    }
